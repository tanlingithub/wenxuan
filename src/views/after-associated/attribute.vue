...
import AssociatedPropertyGroup from './components/associate-property-group'
import {backCategoryProperty} from '../../const/api'
import {
  getPropertyGroups,
  getAssociatedPropertyById,
  changePropertyBatch,
  deletePropertyBatch
} from '../../service/spaas-commodity-center'
...
export default {
  data() {
    const {associatedId} = this.$route.query || {}
    const emptyGroupItem = {
      name: '无',
      id: 0
    }
    const currentUrl = backCategoryProperty({id: associatedId})
    return {
      associatedId,
      emptyGroupItem,
      currentUrl,
  ...
  methods: {
    ...
    // 请求属性组
    getPropertyGroup() {
      getPropertyGroups({categoryId: this.associatedId}).then(res => {
        const {payload} = res
    ...
    saveChange() {
      ...
      const deleteIdStr = delDataList.map(info => info.id).join(',')
      const id = this.associatedId
      let promiseArr = []
      if (changeIdStr) {
        // 更改ID数组
        promiseArr.push(changePropertyBatch(id, changeIdStr, changeDataList))
      }
      if (deleteIdStr) {
        // 删除ID数组
        promiseArr.push(deletePropertyBatch(id, deleteIdStr))
      }

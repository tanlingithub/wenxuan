...
import {frontCategoryProperty} from '@/const/api.js'
import TreeDate from '@/utils/tree-data'
import {postFrontPropertyBatch} from '@/service/spaas-commodity-center'
...
export default {
  ...
  data() {
    const currentUrl = frontCategoryProperty({id: this.categoryId})
    return {
      tableConfig: {
        url: currentUrl,
  ...
  watch: {
    categoryId(val) {
      this.$nextTick(_ => {
        this.tableConfig.url = frontCategoryProperty({id: this.categoryId})
      })
    },
    ifUpdate(val) {
      if (val) {

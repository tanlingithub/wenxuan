...
export default {
  ...
  methods: {
    filterTreeData(keyword) {
      throttle(() => {
        let arr = JSON.parse(JSON.stringify(this.allTreeData))
        if (keyword) {
          let data = this.getDataByKeyword(arr, keyword)
    ...
    getDataByKeyword(arr, keyword) {
      return arr.filter(item => {
        if (item.children) {
          item.children = this.getDataByKeyword(item.children, keyword)
        }

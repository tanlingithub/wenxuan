...
export function redirect(url) {
  let href = location.href.replace(/#.*/, `#${url}`)
  location.replace(href)
}

// 将树结构扁平成一维数组
export function treeToArray(nodes, saveChildren, level = null) {
  var r = []
  if (Array.isArray(nodes)) {
    for (var i = 0, l = nodes.length; i < l; i++) {
      ...
      if (
        ...
        let child = treeToArray(nodes[i]['children'], saveChildren)
        r.push(...child)
      }
      if (!saveChildren) {
